...
(function (DrummerJS, $, undefined) {
  ...
  var calculateBPM = function () {
    _stepDelay = Math.round(
      (_sampleRate * _minuteInSeconds) /
        (_beatsPerMinute * _totalSteps) /
        _totalSteps
    );
    ...
    function _handler() {
      ...
      this.playAudio = function () {
        ...
        for (var i = 1; i < _totalRows + 1; i++) {
          var $_newSample = $(
            '.sample[data-row="' + i + '"][data-column="' + _currentStep + '"]'
          );
      ...
      this.createMatrix = function (rows, columns) {
        ...
        for (var i = 1; i < rows + 1; i++) {
          $('<div class="row" id="row-' + i + '"></div>').appendTo(
            ".container"
          );

          $(
            '<button class="enable-disable-row" id="toggle-row-' +
              i +
              '"></button>'
          ).prependTo("#row-" + i);
          ...
          for (var k = 1; k < columns + 1; k++) {
            $(
              '<div class="column sample" data-row="' +
                i +
                '" data-column="' +
                k +
                '" style="height:' +
